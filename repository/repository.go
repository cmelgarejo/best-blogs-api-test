...
func (e CommentAlreadyExistsError) Error() string {
	return fmt.Sprintf("Comment with id: %v already exists in the database", e.id)
}
...
func (e CommentNotFoundError) Error() string {
	return fmt.Sprintf("Comment with id: %v was not found in the repository!", e.id)
}
...
func (c *CommentRepository) Insert(comment model.Comment) error {
	...
	for i := 0; i < len(c.repository); i++ {
		if c.repository[i].Id == comment.Id {
...
func (c *CommentRepository) GetAllByPostId(id uint64) []model.Comment {
	...
	for i := 0; i < len(c.repository); i++ {
		if c.repository[i].PostId == id {
			comments = append(comments, c.repository[i])
		}
	}
...
func (e PostAlreadyExistsError) Error() string {
	return fmt.Sprintf("Error: Post with id: %v already exists in the database", e.id)
}
